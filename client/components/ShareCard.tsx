...
import { generateKeyPair, getSharedSecret } from "../context/eccutils"
import { encryptFile, decryptFile } from "@/utils/aesutils";
...
const ShareCard = () => {
  ...
  const {peerPublicKey, setPeerPublicKey,der_Key, setDer_Key} = useSocket();
  const [aesKey, setAesKey] = useState<string | null>(null);


  console.log("own public key: ", publicKey);
  ...
  useEffect(() => {
    ...
    userDetails.socket.on("signaling", (data: any) => {
      ...
      console.log("received pub key: ",data.publicKey)
    });
  ...
  useEffect(() => {
    ...
    const deriveAESKey = async () => {
      if (peerPublicKey){
        try {
          const sharedSecret = await getSharedSecret(privateKey, peerPublicKey);
          const derivedKey = sharedSecret.slice(0, 32); // Use first 32 bytes as AES key
          setAesKey(derivedKey);
          setDer_Key(derivedKey);
  ...
  useEffect(() => {
    console.log("peer pubKey Updated:", peerPublicKey);
  },[peerPublicKey])
  
  useEffect(() => {
    console.log("der Key Updated:", der_Key);
  },[der_Key])
  useEffect(() => {
    console.log("AES Key Updated:", aesKey);
  }, [aesKey]);
  ...
  const callUser = () => {
    const peer = new Peer({
      ...
      channelConfig: {
        ordered: true, // Ensure ordered delivery of data
        maxRetransmits: 0, // Ensures no retransmission attempts
      },
    });
...
peer.on("data", async (data) => {
  try {
    ...
    if (parsedData.chunk) {
      setfileReceiving(true);
      if (aesKey === null) {
        throw new Error("AES key should not be null");
      }
      // Decrypt the received chunk
      const decryptedChunk = decryptFile(parsedData.chunk, aesKey);
      console.log("decrypted chunk:",decryptedChunk)
      // Pass the decrypted data to the handler
      handleReceivingData(decryptedChunk);
    } else if (parsedData.done) {
      // Handle the end of the file transfer
      handleReceivingData(parsedData);
      toast.success("File received successfully");
    ...
    const readAndSendChunk = () => {
      ...
      reader.onload = (event) => {
        if (event.target?.result) {
          ...
          const encryptedChunk = encryptFile(chunkData, aesKey);
